...
Keyframe *keyfactory_walk(void *data, bool reverse)
{
    ...
    for (int i = 0; i < *servos_num; i++)
    {
        ...
        if (i == SERVO_INDEX_BACK_LEFT_HIP || i == SERVO_INDEX_FRONT_RIGHT_HIP)
        ...
        if (i == SERVO_INDEX_BACK_RIGHT_HIP || i == SERVO_INDEX_FRONT_LEFT_HIP)
            servo_pos[i] = (ServoPos) { i == SERVO_INDEX_BACK_RIGHT_HIP ? ease_out : ease_in, *hip_delta * mod, -(*hip_delta) * mod, 0.0f, 0.0f };
